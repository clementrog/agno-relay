...
const IDEMPOTENCY_KEY_REQUIRED_MESSAGE = "Mutation failed. Provide X-Idempotency-Key header to enable safe retries.";
/**
 ...
 */
function getToolCallsFromLastMessage(messages) {
...
function serializeToolResult(result) {
    ...
    if (typeof result === "object" && result !== null && "content" in result) {
        ...
        if (Array.isArray(content)) {
            const texts = content
                .filter((c) => c != null && typeof c === "object" && "text" in c)
                .map((c) => (typeof c.text === "string" ? c.text : ""));
            if (texts.length > 0)
                return JSON.stringify({ result: texts.join("\n") });
        }
    }
 ...
 */
export async function handleChatCompletion(request, bridge, options) {
    ...
    for (const tc of toolCalls) {
        ...
        try {
            ...
            const isTimeout = /timeout|deadline/i.test(message);
