...
const ERROR_CLASS_TO_ACTION: Record<ErrorClass, string> = {
...
export function createCanonicalError(
  ...
  const suggested_backoff_ms =
    options?.suggested_backoff_ms !== undefined
 ...
 */
export function httpStatusToErrorClass(status: number): ErrorClass {
  switch (status) {
    case 401:
      return "auth";
    case 403:
 ...
 */
export function wrapUpstreamError(response: UpstreamResponse): CanonicalError {
  const status = response.status;
  const errorClass = httpStatusToErrorClass(status);
  const context: CanonicalErrorContext = { source: "upstream", upstream_code: status };
  ...
  let suggested_backoff_ms: number | null = errorClass === "transient" ? 1000 : null;
...
export function wrapNetworkError(
  message: string,
  context: CanonicalErrorContext = { source: "bridge" }
): CanonicalError {
 ...
 */
export function setRetryable(
  error: CanonicalError,
  options: { isReadOnly?: boolean; hasIdempotencyKey?: boolean }
): CanonicalError {
